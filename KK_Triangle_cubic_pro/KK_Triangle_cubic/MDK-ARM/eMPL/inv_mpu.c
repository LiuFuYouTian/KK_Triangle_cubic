...
/****************************************************************************	
	设置数据已经准备好触发中断
	注意如果DMP功能已经打开，那么触发的是DMP完成中断，
	当DMP功能未打开时，那么将触发数据已经准备好中断
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            tmp = BIT_DMP_INT_EN;					//使能DMP完成中断
        else
            tmp = 0x00;								//关闭所有中断
		//将中断使能信息配置到中断使能（0x38）寄存器中，
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
        st.chip_cfg.int_enable = tmp;				//保存开启了哪个中断的信息
        ...
        if (enable && st.chip_cfg.int_enable)
            return 0;
        if (enable)
            tmp = BIT_DATA_RDY_EN;					//当DMP功能未打开时，使能数据已准备好中断
        else
            tmp = 0x00;								//关闭所有中断
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
/**************************************************************************
	MPU初始化（注意：这里只是赋予初始值，后续可能会对某些设置值进行更改）：
	陀螺仪量程：			±2000DPS
	加速计量程：			±2G
	数字低通滤波带宽值：	42Hz
	FIFO读取频率：			50Hz
...
int mpu_init(void)															//这里直接将参数删除即可
{
    ...
    /* Wake up chip.唤醒MPU，并注意此时MPU6050使用的是内部时钟 */
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;

#if defined MPU6050
    /* Check product revision. 检查版本号*/
...
#define MPU6500_MEM_REV_ADDR    (0x17)
	 ...
	 * mpu_set_sensors函数中会保存这些设置值
	 */
    st.chip_cfg.clk_src = INV_CLK_PLL;				//使用PLL时钟源作为MPU的时钟，以提高精度
    /* Handled in next call to mpu_set_bypass. */
    st.chip_cfg.active_low_int = 1;					//保存INT引脚为为低有效配置信息
    st.chip_cfg.latched_int = 0;						//保存INT引脚产生50us的脉冲配置信息
    st.chip_cfg.int_motion_only = 0;				//保存只触发运动中断关闭配置信息
    st.chip_cfg.lp_accel_mode = 0;					//保存低功耗加速计模式关闭配置信息
    memset(&st.chip_cfg.cache, 0, sizeof(st.chip_cfg.cache));
    st.chip_cfg.dmp_on = 0;								//保存DMP功能未打开的配置信息
    st.chip_cfg.dmp_loaded = 0;						//保存DMP功能未加载的配置信息（确保DMP只被加载一次）
    st.chip_cfg.dmp_sample_rate = 0;			//保存dmp采样频率尚未设置信息

    if (mpu_set_gyro_fsr(GYRO_FSR))
        return -1;
    if (mpu_set_accel_fsr(ACCEL_FSR))
        return -1;
    if (mpu_set_lpf(LPF))							//设置数字低通滤波功能，这里设置的带宽值为42，注意带宽值一般近似等于采样率的一半
        return -1;
    if (mpu_set_sample_rate(MPU_SAMPLE_RATE))		//设置MPU的采样率，这里设置是50Hz(默认配置，采样率在后面会被重新配置)
        return -1;
    if (mpu_configure_fifo(0))						//初始化FIFO，0表示此处不配置哪些数据可以进入FIFO中
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
	...
	//当已经打开了DMP时，不能再配置采样率了
    if (st.chip_cfg.dmp_on)
        return -1;
    else {
		//配置低功耗模式，并且注意此模式下，采样率范围为：0~40Hz
		...
		//将采样率配置到采样频率分频寄存器（0x19）中
        if (i2c_write(st.hw->addr, st.reg->rate_div, 1, &data))
            return -1;

        st.chip_cfg.sample_rate = 1000 / (1 + data);	//保存采样率配置信息
...
/************************************************************
	选择哪些数据可以进入FIFO中，注意，陀螺仪各轴的数据可以独立的进入
	FIFO相应的区域，但是加速计只能是三轴一起进入FIFO中，并且打开DMP完成中断
    ...
    unsigned char *more)
{
    ...
    if (!st.chip_cfg.dmp_on)										//当DMP没有打开时直接返回-1
        return -1;
    if (!st.chip_cfg.sensors)										//当未配置使用传感器时直接返回-1
        return -1;

    if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp))		//读取FIFO计数寄存器中的值，从而得到当前FIFO中保存的数据量的大小
        return -1;
	...
	//判断FIFO中保存的数据是否已经过半了，表明数据可能已经不再有效期内了，并且有可能FIFO已经溢出（注意：FIFO最大能保存的数据量是1KB）
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read(st.hw->addr, st.reg->int_status, 1, tmp))
            return -1;
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();					//FIFO中保存的数据已经达到最大值的一半以上了，需要重置FIFO缓冲区，以保证数据的时效性
            return -2;
        }
    }
	
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data))		//读取FIFO中保存的数据
        return -1;
    more[0] = fifo_count / length - 1;								//获得FIFO中还剩余的数据量
    return 0;
}

/**********************************************************************
	设置旁路模式，这个在使用到磁力计的时候才用的到
...
/***********************************************************************
	此函数是配置INT引脚在中断发生时是产生50us的脉冲，
	还是产生高电平直到中断被清除的模式
	enable :0时，则是产生50us的脉冲，1时，则是产生高电平直到中断被清除
...
int mpu_set_int_latched(unsigned char enable)
{
	...
	//根据latched_int中的值来判断是否已经设置了和传入的参数值一致
	...
	//配置主机是否可以直接访问MPU6050的辅助I2C总线，这个当用到磁力计时有用
	...
	//将以上的配置写入int_pin_cfg（0x37）寄存器中
    if (i2c_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
        return -1;
	
	//将latched_int指示标志位置位，表明已经配置过中断了
...
int mpu_run_self_test(long *gyro, long *accel)
{
	...
	//判断DMP是否已经被打开，是则先关闭DMP
    if (st.chip_cfg.dmp_on) {
        mpu_set_dmp_state(0);
        dmp_was_on = 1;				//保存在这之前DMP的状态值
    } else
        dmp_was_on = 0;
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(gyro, accel, 0))			//获得陀螺仪和加速计的第一次偏差
            break;
    if (ii == tries) {
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(gyro_st, accel_st, 1))	//获得第二次偏差
            break;
    if (ii == tries) {
...
restore:
     ...
     */
    get_st_biases(gyro, accel, 0);	//返回陀螺仪和加速计的偏差值
    result = 0x7;					//所有传感器均测试通过，将返回0x7	
#endif
    /* Set to invalid values to ensure no I2C writes are skipped. */
    st.chip_cfg.gyro_fsr = 0xFF;
    st.chip_cfg.accel_fsr = 0xFF;
    ...
    return result;		//返回陀螺仪和加速计的自检结果，注意，返回的还有检测到的偏差值，以便后续用于校准
}
...
#define LOAD_CHUNK  (16)
    ...
    st.chip_cfg.dmp_loaded = 1;								//保存dmp固件已加载的配置信息，防止重复加载
    st.chip_cfg.dmp_sample_rate = sample_rate;				//保存DMP采样频率，这里为200Hz（默认配置）（这个频率配置会被后面的配置所覆盖掉）
    return 0;
}
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        set_int_enable(0);									//关闭所有中断
        /* Disable bypass mode. */
        mpu_set_bypass(0);									//关闭旁路模式
        /* Keep constant sample rate, FIFO rate controlled by DMP. */
        mpu_set_sample_rate(st.chip_cfg.dmp_sample_rate);	//设置MPU采样率和DMP的输出采样率一致
        /* Remove FIFO elements. */
        tmp = 0;
        i2c_write(st.hw->addr, 0x23, 1, &tmp);
        st.chip_cfg.dmp_on = 1;
        /* Enable DMP interrupt. */
        set_int_enable(1);									//开启DMP完成中断
...
/* This initialization is similar to the one in ak8975.c. */					//这里由于没有使用磁力计，所以全部注释掉即可
//static int setup_compass(void)
//{
//#ifdef AK89xx_SECONDARY
